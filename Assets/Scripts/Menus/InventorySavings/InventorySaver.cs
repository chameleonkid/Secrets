...
public class InventorySaver : MonoBehaviour
{
    ...
    {
        Debug.Log("Import Save Data " + SL.serialiableList.Count);
        //go through the Sl and rebuild the items in the inventory
        for (int i = 0; i < SL.serialiableList.Count; i++)
        {

            //we will need the name and the count from the save data
            string name = SL.serialiableList[i].name;
            int count = SL.serialiableList[i].count;
